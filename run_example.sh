...
do
    ...
    python3 src/feature_extractor.py -r histogram benchmarks_train/$b/ -s config_files/$sequence_filename -o ignore/hist # > /dev/null
    python3 src/feature_extractor.py -r ir2vec benchmarks_train/$b/ -s config_files/$sequence_filename -o ignore/ir2vec # > /dev/null
    #Collect program speedup 
    python3 src/speedup_collector.py benchmarks_train/$b/ -s config_files/$sequence_filename -o ignore/speedups/ # > /dev/null
    #collect program runtime
    python3 src/runtime_collector.py benchmarks_train/$b/ -s config_files/$sequence_filename -o ignore/runtime # > /dev/null
    #collect number of cycles
    python3 src/cycles_collector.py benchmarks_train/$b/ -s config_files/$sequence_filename -o ignore/cycles # > /dev/null
